...
var enableNewReconciler = true; // Flow magic to verify the exports of this file match the original version.
...
var IS_REPLAYED = 1 << 4;
var IS_LEGACY_FB_SUPPORT_MODE = 1 << 5;
var SHOULD_NOT_DEFER_CLICK_FOR_FB_SUPPORT_MODE =
  IS_LEGACY_FB_SUPPORT_MODE | IS_REPLAYED | IS_CAPTURE_PHASE; // We do not want to defer if the event system has already been
...
var StaticMask =
  /*                   */
  32768; // These flags allow us to traverse to fibers that have effects on mount
...
function dispatchDiscreteEvent(
  ...
  if (
    !enableLegacyFBSupport || // If we are in Legacy FB support mode, it means we've already
...
function extractEvents$3(
  ...
  if (!enableEagerRootListeners) {
    var eventListenerSet = getEventListenerSet(targetContainer); // Track whether all listeners exists for this plugin. If none exist, we do
...
function listenToReactEvent(reactEvent, rootContainerElement, targetElement) {
  if (!enableEagerRootListeners) {
    ...
    if (isPolyfillEventPlugin) {
      var listenerSet = getEventListenerSet(rootContainerElement); // When eager listeners are off, this Set has a dual purpose: it both
      ...
      var isCapturePhaseListener =
        reactEvent.substr(-7) === "Capture" && // Edge case: onGotPointerCapture and onLostPointerCapture
  ...
  if (!enableEagerRootListeners) {
...

    {
      if (
        ...
        (rootContainerElement.nodeType !== ELEMENT_NODE && // This is to support rendering into a ShadowRoot:
...
function markCommitStarted(lanes) {
  {
    if (supportsUserTiming) {
      performance.mark("--commit-start-" + formatLanes$1(lanes));
    }
  }
}
...
function markLayoutEffectsStarted(lanes) {
  {
    if (supportsUserTiming) {
      performance.mark("--layout-effects-start-" + formatLanes$1(lanes));
    }
  }
}
...
function markPassiveEffectsStarted(lanes) {
  {
    if (supportsUserTiming) {
      performance.mark("--passive-effects-start-" + formatLanes$1(lanes));
    }
  }
}
...
function markRenderStarted(lanes) {
  {
    if (supportsUserTiming) {
      performance.mark("--render-start-" + formatLanes$1(lanes));
    }
  }
}
...
function markRenderScheduled(lane) {
  {
    if (supportsUserTiming) {
      performance.mark("--schedule-render-" + formatLanes$1(lane));
    }
  }
}
...
function completeWork(current, workInProgress, renderLanes) {
  ...
  switch (workInProgress.tag) {
    ...
    case Profiler: {
      var didBailout = bubbleProperties(workInProgress);
      ...
      if (!didBailout) {
        ...
        var OnRenderFlag = Update;
        var OnCommitFlag = Callback;
        var OnPostCommitFlag = Passive;
        var subtreeFlags = workInProgress.subtreeFlags;
        var flags = workInProgress.flags;
        var newFlags = flags; // Call onRender any time this fiber or its subtree are worked on.
...
function recursivelyCommitLayoutEffects(finishedWork, finishedRoot) {
  ...
  switch (tag) {
    case Profiler: {
      ...
      while (child !== null) {
        var primarySubtreeFlags = finishedWork.subtreeFlags & LayoutMask;

        if (primarySubtreeFlags !== NoFlags) {
          {
            var prevCurrentFiberInDEV = current;
            ...
            if (prevCurrentFiberInDEV !== null) {
              setCurrentFiber(prevCurrentFiberInDEV);
            } else {
              resetCurrentFiber();
            }
          }
      ...
      var primaryFlags = flags & (Update | Callback);

      if (primaryFlags !== NoFlags) {
        {
          {
            var _prevCurrentFiberInDEV = current;
            ...
            if (_prevCurrentFiberInDEV !== null) {
              setCurrentFiber(_prevCurrentFiberInDEV);
            } else {
              resetCurrentFiber();
            }
          }
    ...
    default: {
      var _child = finishedWork.child;

      while (_child !== null) {
        var _primarySubtreeFlags = finishedWork.subtreeFlags & LayoutMask;

        if (_primarySubtreeFlags !== NoFlags) {
          {
            var _prevCurrentFiberInDEV2 = current;
            setCurrentFiber(_child);
            invokeGuardedCallback(
              null,
              recursivelyCommitLayoutEffects,
              null,
            ...
            if (hasCaughtError()) {
              ...
              captureCommitPhaseError(_child, finishedWork, _error2);
            }

            if (_prevCurrentFiberInDEV2 !== null) {
              setCurrentFiber(_prevCurrentFiberInDEV2);
            } else {
              resetCurrentFiber();
            }
          }
      ...
      var _primaryFlags = flags & (Update | Callback);

      if (_primaryFlags !== NoFlags) {
        switch (tag) {
          case FunctionComponent:
          case ForwardRef:
          case SimpleMemoComponent:
...
function commitLayoutEffectsForHostComponent(finishedWork) {
  var instance = finishedWork.stateNode;
  var current = finishedWork.alternate; // Renderers may schedule work to be done after host components are mounted
...
var RENDER_TIMEOUT_MS = 500; // Used to avoid traversing the return path to find the nearest Profiler ancestor during commit.

var nearestProfilerOnStack$1 = null;
...
function requestUpdateLane(fiber) {
  ...
  if (
    ...
    if (decoupleUpdatePriorityFromScheduler) {
      ...
      var currentUpdateLanePriority = getCurrentUpdateLanePriority();
...
function commitBeforeMutationEffectsDeletions(deletions) {
  for (var i = 0; i < deletions.length; i++) {
    var _fiber = deletions[i]; // TODO (effects) It would be nice to avoid calling doesFiberContain()
    // Maybe we can repurpose one of the subtreeFlags positions for this instead?
    // Use it to store which part of the tree the focused instance is in?
    // This assumes we can safely determine that instance during the "render" phase.

    if (doesFiberContain(_fiber, focusedInstanceHandle)) {
      shouldFireAfterActiveInstanceBlur = true;
      beforeActiveInstanceBlur();
    }
  }
...
function flushPassiveMountEffects(root, firstChild) {
  ...
  while (fiber !== null) {
    ...
    if ((fiber.flags & Passive) !== NoFlags) {
      {
        setCurrentFiber(fiber);
        invokeGuardedCallback(null, commitPassiveMount, null, root, fiber);
...
function invokeEffectsInDev(firstChild, fiberFlags, invokeEffectFn) {
  if (enableDoubleInvokingEffects) {
    var _fiber2 = firstChild;

    while (_fiber2 !== null) {
      if (_fiber2.child !== null) {
        var primarySubtreeFlag = _fiber2.subtreeFlags & fiberFlags;

        if (primarySubtreeFlag !== NoFlags) {
          invokeEffectsInDev(_fiber2.child, fiberFlags, invokeEffectFn);
        }
      }

      if ((_fiber2.flags & fiberFlags) !== NoFlags) {
        invokeEffectFn(_fiber2);
      }

      _fiber2 = _fiber2.sibling;
    }
  }
}
    ...
    if (hydrate && tag !== LegacyRoot) {
      var doc =
        containerNodeType === DOCUMENT_NODE
          ? container
          : container.ownerDocument; // We need to cast this because Flow doesn't work
...
function registerEventOnNearestTargetContainer(
  ...
  if (!enableEagerRootListeners) {
    ...
    var targetContainer = getNearestRootOrPortalContainer(targetFiber);
...
function registerReactDOMEvent(target, domEventName, isCapturePhaseListener) {
  ...
  if (target.nodeType === ELEMENT_NODE) {
    if (!enableEagerRootListeners) {
      var targetElement = target; // Check if the DOM element is managed by React.

      var targetFiber = getClosestInstanceFromNode(targetElement);
    ...
    if (!enableEagerRootListeners) {
      var scopeTarget = target;

      var _targetFiber = getFiberFromScopeInstance(scopeTarget);

      if (_targetFiber === null) {
