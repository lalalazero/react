...
if (canUseDOM) {
  var JSCompiler_inline_result$jscomp$165;
  if (canUseDOM) {
    var isSupported$jscomp$inline_288 = "oninput" in document;
    if (!isSupported$jscomp$inline_288) {
      var element$jscomp$inline_289 = document.createElement("div");
      element$jscomp$inline_289.setAttribute("oninput", "return;");
      isSupported$jscomp$inline_288 =
        "function" === typeof element$jscomp$inline_289.oninput;
    }
    JSCompiler_inline_result$jscomp$165 = isSupported$jscomp$inline_288;
  } else JSCompiler_inline_result$jscomp$165 = !1;
  isInputEventSupported =
    JSCompiler_inline_result$jscomp$165 &&
...
for (
  var i$jscomp$inline_1322 = 0;
  i$jscomp$inline_1322 < otherDiscreteEvents.length;
  i$jscomp$inline_1322++
)
  eventPriorities.set(otherDiscreteEvents[i$jscomp$inline_1322], 0);
...
function flushSyncCallbackQueueImpl() {
  if (!isFlushingSyncQueue && null !== syncQueue) {
    ...
    if (decoupleUpdatePriorityFromScheduler) {
      ...
      try {
        var queue$35 = syncQueue;
        runWithPriority$1(99, function() {
          for (; i < queue$35.length; i++) {
            var callback = queue$35[i];
            do callback = callback(!0);
            while (null !== callback);
          }
        });
...
var supportsUserTiming =
  "undefined" !== typeof performance && "function" === typeof performance.mark;
supportsUserTiming && performance.mark("--react-init-17.0.0");
function markCommitStopped() {
  supportsUserTiming && performance.mark("--commit-stop");
}
...
function markComponentSuspended(fiber, wakeable) {
  if (supportsUserTiming) {
    var id = getWakeableID(wakeable),
      componentName = getComponentName(fiber.type) || "Unknown";
...
function markRenderStarted(lanes) {
  supportsUserTiming && performance.mark("--render-start-" + lanes.toString());
}
function markRenderStopped() {
  supportsUserTiming && performance.mark("--render-stop");
}
function markStateUpdateScheduled(fiber, lane) {
  supportsUserTiming &&
    ((fiber = getComponentName(fiber.type) || "Unknown"),
    performance.mark(
      "--schedule-state-update-" + lane.toString() + "-" + fiber
    ));
}
...
var classComponentUpdater = {
  ...
  enqueueForceUpdate: function(inst, callback) {
    ...
    scheduleUpdateOnFiber(fiber, inst, eventTime);
    supportsUserTiming &&
      ((callback = getComponentName(fiber.type) || "Unknown"),
      performance.mark(
        "--schedule-forced-update-" + inst.toString() + "-" + callback
      ));
  }
};
  ...
  switch (workInProgress.tag) {
    ...
    case 4:
      return (
        ...
        updateHostContainer(workInProgress),
        null === current &&
          ((workInProgress = workInProgress.stateNode.containerInfo),
          enableEagerRootListeners
            ? listenToAllSupportedEvents(workInProgress)
            : listenToReactEvent("onMouseEnter", workInProgress, null)),
        null
      );
    case 10:
    ...
    case 19:
      ...
      type = 0 !== (workInProgress.flags & 64);
                ...
                for (newProps = workInProgress.child; null !== newProps; )
                  (type = newProps),
                    ...
                    null === props
                      ...
                      : ((type.childLanes = props.childLanes),
                        (type.lanes = props.lanes),
...
function performConcurrentWorkOnRoot(root) {
  ...
  if (
    ...
    resetRenderTimer(),
      ...
      startWorkOnPendingInteractions(root, exitStatus);
  var prevInteractions = pushInteractions(root);
  markRenderStarted(exitStatus);
  do
    try {
      workLoopConcurrent();
      break;
  ...
  null !== workInProgress
    ? (supportsUserTiming && performance.mark("--render-yield"),
      (exitStatus = 0))
    : (markRenderStopped(),
      (workInProgressRoot = null),
      (workInProgressRootRenderLanes = 0),
      ...
      case 4:
        ...
        for (prevExecutionContext = -1; 0 < lanes; )
          (prevInteractions = 31 - clz32(lanes)),
            ...
            (prevInteractions = exitStatus[prevInteractions]),
...
function commitRootImpl(root, renderPriorityLevel) {
  ...
  supportsUserTiming && performance.mark("--commit-start-" + lanes.toString());
  ...
  if (null !== remainingLanes) {
    ...
    if (hasSelectionCapabilities(expirationTimes)) {
      if ("selectionStart" in expirationTimes)
        noLongerPendingLanes = {
          start: expirationTimes.selectionStart,
          end: expirationTimes.selectionEnd
        };
      else
        a: {
          noLongerPendingLanes =
            ((noLongerPendingLanes = expirationTimes.ownerDocument) &&
              noLongerPendingLanes.defaultView) ||
            window;
          var selection =
            noLongerPendingLanes.getSelection &&
            noLongerPendingLanes.getSelection();
          if (selection && 0 !== selection.rangeCount) {
            noLongerPendingLanes = selection.anchorNode;
            index$21 = selection.anchorOffset;
            ...
            b: for (;;) {
              for (var next; ; ) {
                node !== noLongerPendingLanes ||
                  (0 !== index$21 && 3 !== node.nodeType) ||
                  (start = length + index$21);
                node !== lane ||
              ...
              for (;;) {
                if (node === expirationTimes) break b;
                parentNode === noLongerPendingLanes &&
                  ++indexWithinAnchor === index$21 &&
                  (start = length);
                parentNode === lane &&
          ...
          } else noLongerPendingLanes = null;
        }
      noLongerPendingLanes = noLongerPendingLanes || { start: 0, end: 0 };
    } else noLongerPendingLanes = null;
    selectionInformation = {
      focusedElem: expirationTimes,
      selectionRange: noLongerPendingLanes
    };
    expirationTimes = null;
    noLongerPendingLanes = selectionInformation.focusedElem;
    null !== noLongerPendingLanes &&
      (expirationTimes = getClosestInstanceFromNode(noLongerPendingLanes));
    _enabled = !1;
    focusedInstanceHandle = expirationTimes;
    shouldFireAfterActiveInstanceBlur = !1;
    ...
    do
      try {
        ...
        if (null === nextEffect) throw Error(formatProdErrorMessage(330));
        captureCommitPhaseError(nextEffect, error$129);
        nextEffect = nextEffect.nextEffect;
      }
    while (null !== nextEffect);
    shouldFireAfterActiveInstanceBlur &&
      ((_enabled = !0),
      (flags = selectionInformation.focusedElem),
      (current = createEvent("afterblur", !1)),
      (current.relatedTarget = flags),
      document.dispatchEvent(current),
      (_enabled = !1));
    currentRef = selectionInformation;
    current = getActiveElementDeep();
    flags = currentRef.focusedElem;
    expirationTimes = currentRef.selectionRange;
    if (
      current !== flags &&
      flags &&
      flags.ownerDocument &&
      ...
      null !== expirationTimes &&
        hasSelectionCapabilities(flags) &&
        ((current = expirationTimes.start),
        (currentRef = expirationTimes.end),
        ...
        "selectionStart" in flags
          ...
          : ((currentRef =
            ...
            currentRef.getSelection &&
              ...
              (noLongerPendingLanes = flags.textContent.length),
              (alternate = Math.min(
                expirationTimes.start,
                noLongerPendingLanes
              )),
              (expirationTimes =
                void 0 === expirationTimes.end
                  ? alternate
                  : Math.min(expirationTimes.end, noLongerPendingLanes)),
              !currentRef.extend &&
                alternate > expirationTimes &&
                ((noLongerPendingLanes = expirationTimes),
                (expirationTimes = alternate),
                (alternate = noLongerPendingLanes)),
              (noLongerPendingLanes = getNodeForCharacterOffset(
                flags,
                alternate
              )),
              (index$21 = getNodeForCharacterOffset(flags, expirationTimes)),
              noLongerPendingLanes &&
                index$21 &&
                (1 !== currentRef.rangeCount ||
                  currentRef.anchorNode !== noLongerPendingLanes.node ||
                  currentRef.anchorOffset !== noLongerPendingLanes.offset ||
                  currentRef.focusNode !== index$21.node ||
                  currentRef.focusOffset !== index$21.offset) &&
                ...
                alternate > expirationTimes
                  ? (currentRef.addRange(current),
                    currentRef.extend(index$21.node, index$21.offset))
                  : (current.setEnd(index$21.node, index$21.offset),
                    currentRef.addRange(current))))));
      current = [];
    ...
    do
      try {
        ...
        for (
          supportsUserTiming &&
          performance.mark("--layout-effects-start-" + lanes.toString());
          null !== nextEffect;
        ...
        supportsUserTiming && performance.mark("--layout-effects-stop");
      } catch (error$130) {
        if (null === nextEffect) throw Error(formatProdErrorMessage(330));
        captureCommitPhaseError(nextEffect, error$130);
        nextEffect = nextEffect.nextEffect;
      }
    while (null !== nextEffect);
    nextEffect = null;
  ...
  if ((previousLanePriority = rootDoesHavePassiveEffects))
    ...
    for (nextEffect = remainingLanes; null !== nextEffect; )
      (flags$jscomp$0 = nextEffect.nextEffect),
        (nextEffect.nextEffect = null),
        nextEffect.flags & 8 &&
          ((remainingLanes = nextEffect),
          (remainingLanes.sibling = null),
          (remainingLanes.stateNode = null)),
        (nextEffect = flags$jscomp$0);
  remainingLanes = root.pendingLanes;
  if (0 !== remainingLanes) {
    if (null !== spawnedWorkDuringRender)
      for (
        flags$jscomp$0 = spawnedWorkDuringRender,
          spawnedWorkDuringRender = null,
          remainingLanes$jscomp$0 = 0;
        remainingLanes$jscomp$0 < flags$jscomp$0.length;
        remainingLanes$jscomp$0++
      )
        scheduleInteractions(
          root,
          flags$jscomp$0[remainingLanes$jscomp$0],
  ...
  if (hasUncaughtError)
    ...
    (firstUncaughtError = null),
    root);
  if (0 !== (executionContext & 8)) return markCommitStopped(), null;
  flushSyncCallbackQueue();
  markCommitStopped();
  return null;
}
...
function flushPassiveEffects() {
  if (90 !== pendingPassiveEffectsRenderPriority) {
    ...
    } else return runWithPriority$1(priorityLevel, flushPassiveEffectsImpl);
  }
  return !1;
}
...
function flushPassiveEffectsImpl() {
  ...
  supportsUserTiming &&
    performance.mark("--passive-effects-start-" + lanes.toString());
  var prevExecutionContext = executionContext;
  executionContext |= 32;
  var prevInteractions = pushInteractions(root),
  ...
  for (var i = 0; i < unmountEffects.length; i += 2) {
    var effect$135 = unmountEffects[i],
      fiber = unmountEffects[i + 1],
      destroy = effect$135.destroy;
    effect$135.destroy = void 0;
  ...
  for (i = 0; i < unmountEffects.length; i += 2) {
    effect$135 = unmountEffects[i];
    fiber = unmountEffects[i + 1];
    try {
      var create = effect$135.create;
      if (fiber.mode & 8)
        try {
          (passiveEffectStartTime = now$1()), (effect$135.destroy = create());
        } finally {
          recordPassiveEffectDuration(fiber);
        }
      else effect$135.destroy = create();
    } catch (error$139) {
      if (null === fiber) throw Error(formatProdErrorMessage(330));
      captureCommitPhaseError(fiber, error$139);
    }
  }
  ...
  for (unmountEffects = 0; unmountEffects < create.length; unmountEffects++)
    if (((effect$135 = create[unmountEffects]), 0 !== (effect$135.flags & 4)))
      switch (effect$135.tag) {
        case 12:
          i = effect$135.stateNode.passiveEffectDuration;
          destroy = effect$135.memoizedProps;
          fiber = destroy.id;
          destroy = destroy.onPostCommit;
          var commitTime$118 = commitTime;
          "function" === typeof destroy &&
            destroy(
              fiber,
              null === effect$135.alternate ? "mount" : "update",
              i,
              commitTime$118,
              root.memoizedInteractions
            );
          for (effect$135 = effect$135.return; null !== effect$135; ) {
            if (12 === effect$135.tag) {
              effect$135.stateNode.passiveEffectDuration += i;
              break;
            }
            effect$135 = effect$135.return;
          }
      }
  tracing.__interactionsRef.current = prevInteractions;
  finishPendingInteractions(root, lanes);
  supportsUserTiming && performance.mark("--passive-effects-stop");
  executionContext = prevExecutionContext;
  flushSyncCallbackQueue();
  return !0;
}
...
function updateContainer(element, container, parentComponent, callback) {
  ...
  supportsUserTiming &&
    performance.mark("--schedule-render-" + lane.toString());
...
attemptSynchronousHydration = function(fiber) {
  switch (fiber.tag) {
    case 3:
      var root$144 = fiber.stateNode;
      if (root$144.hydrate) {
        var lanes = getHighestPriorityLanes(root$144.pendingLanes);
        root$144.expiredLanes |= lanes & root$144.pendingLanes;
        ensureRootIsScheduled(root$144, now());
  ...
  if (decoupleUpdatePriorityFromScheduler) {
    ...
    try {
      return (
        ...
        runWithPriority$1(98, fn.bind(null, a, b, c, d))
      );
    } finally {
      (currentUpdateLanePriority = previousLanePriority),
        (executionContext = prevExecutionContext),
...
var Internals = {
  ...
  devToolsConfig$jscomp$inline_1277 = {
    findFiberByHostInstance: getClosestInstanceFromNode,
    bundleType: 0,
    version: "17.0.0",
    rendererPackageName: "react-dom"
  };
var internals$jscomp$inline_1611 = {
  bundleType: devToolsConfig$jscomp$inline_1277.bundleType,
  version: devToolsConfig$jscomp$inline_1277.version,
  rendererPackageName: devToolsConfig$jscomp$inline_1277.rendererPackageName,
  rendererConfig: devToolsConfig$jscomp$inline_1277.rendererConfig,
  ...
  findFiberByHostInstance:
    devToolsConfig$jscomp$inline_1277.findFiberByHostInstance ||
...
if ("undefined" !== typeof __REACT_DEVTOOLS_GLOBAL_HOOK__) {
  var hook$jscomp$inline_1612 = __REACT_DEVTOOLS_GLOBAL_HOOK__;
  if (
    !hook$jscomp$inline_1612.isDisabled &&
    hook$jscomp$inline_1612.supportsFiber
  )
    try {
      (rendererID = hook$jscomp$inline_1612.inject(
        internals$jscomp$inline_1611
      )),
        (injectedHook = hook$jscomp$inline_1612);
    } catch (err) {}
}
...
exports.unstable_flushControlled = function(fn) {
  ...
  if (decoupleUpdatePriorityFromScheduler) {
    var previousLanePriority = currentUpdateLanePriority;
    try {
      (currentUpdateLanePriority = 15), runWithPriority$1(99, fn);
    } finally {
      (currentUpdateLanePriority = previousLanePriority),
        (executionContext = prevExecutionContext),
        0 === executionContext &&
